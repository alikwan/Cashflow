// البيت السعيد — Primitives (Icons + reusable UI). Plain React via Babel.
/* global React */
...
function Icon({ name, className = "w-5 h-5", strokeWidth = 1.5, style }) {
  // لا يوجد Tailwind في هذا المشروع — نشتق الأبعاد بالبكسل من أصناف w-/h-
  let w = 20, h = 20;
  const mw = /w-([\d.]+)/.exec(className); if (mw && TW_PX[mw[1]]) w = TW_PX[mw[1]];
  const mh = /h-([\d.]+)/.exec(className); if (mh && TW_PX[mh[1]]) h = TW_PX[mh[1]];
  return (
...
// ---------- Badge --------------------------------------------------------
function Badge({ tone = "gray", children, dot = false, className = '', style }) {
  return (
...
// ---------- Button -------------------------------------------------------
function Button({ variant = 'primary', size = 'md', icon, iconEnd, children, onClick, disabled, className = '', style, title }) {
...
function IconButton({ name, onClick, title, size = 32, active = false, style }) {
...
// ---------- Segmented tabs ----------------------------------------------
function SegmentedTabs({ items, value, onChange, size = 'md' }) {
...
// ---------- Card + section ----------------------------------------------
function Card({ children, padding = 20, className = '', style, onClick }) {
...
function StatCard({ label, value, unit, sub, icon, tone = 'primary', trend, onClick, footer }) {
  const tints = {
...
function MiniStat({ label, value, unit, tone = 'primary', icon, note }) {
  const tints = {
...
// ---------- Toast system ------------------------------------------------
const toastBus = { fns: [], emit(t) { this.fns.forEach(f => f(t)); }, on(f) { this.fns.push(f); return () => { this.fns = this.fns.filter(x => x !== f); }; } };
window.showToast = (message, tone = 'success') => toastBus.emit({ id: Date.now() + Math.random(), message, tone });
function ToastHost() {
...
function NumberField({ value, onChange, min, max, step = 1, suffix, width = 110 }) {
  return (
...
Object.assign(window, {
  Icon, Badge, Button, IconButton, SegmentedTabs, Card, SectionHeader, StatCard, MiniStat, useFloatTip,
  ToastHost, Toggle, NumberField, Slider, SettingRow,
});
