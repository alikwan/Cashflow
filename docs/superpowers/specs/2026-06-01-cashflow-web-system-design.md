...
**`installments_aging`** — **⏳ قيد فحص أعمق** (الفحص الأولي لم يجد حقول استحقاق في `Premiums`/`PremiumPays`، لكن المالك يؤكّد أن الأقساط مُدارة جيداً في المصدر — فالبيانات موجودة على الأرجح في جدول آخر؛ يلزم مسح شامل قبل أي قرار، انظر `discovery/01`). التصميم: PK مركّب **`(snapshot_date, bucket_key)`**، `label text`, `amount_m`, `count int`؛ يفرض ETL Σ(amount_m) = `installments_summary.remaining_m`.
...
- **أعمار الأقساط**: ⏳ **قيد فحص أعمق** — الفحص الأولي (أعمدة `Premiums`/`PremiumPays` فقط) لم يجد تواريخ استحقاق، لكن المالك يؤكّد أن الأقساط مُدارة جيداً في المصدر؛ يلزم مسح شامل لكل الجداول (عبر `BillId`/جدول تفاصيل/جدولة) لتحديد مصدر الاستحقاقات **قبل** أي قرار بالإسقاط (`discovery/01`).
...
| حقول تاريخ استحقاق الأقساط | ⏳ **فحص أعمق مطلوب** (`discovery/01`): الفحص الأولي لم يجدها، لكن المالك يؤكّد وجودها — يلزم مسح كل جداول القاعدة |
