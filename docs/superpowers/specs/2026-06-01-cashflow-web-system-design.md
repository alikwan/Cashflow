...
**`installments_aging`** — ✅ **قابل للبناء** (الاستحقاقات **مشتقّة**: `InitDate` + جدولة شهرية × `PremiumPayAmount`، دفعات FIFO — `discovery/01`؛ يبقى تعريف مقياس العمر بانتظار اختيار المالك). PK مركّب **`(snapshot_date, bucket_key)`**، `label text`, `amount_m`, `count int`؛ يفرض ETL Σ(amount_m) = `installments_summary.remaining_m`.
...
- **أعمار الأقساط**: ✅ **محلولة، قابلة للبناء** — الاستحقاقات مشتقّة من `InitDate` + جدولة شهرية (`PremiumType=1`) × `PremiumPayAmount` ودفعات FIFO (`discovery/01`). يبقى **اختيار تعريف العمر** مع المالك (أيام منذ آخر دفعة / جدولة صارمة / مزيج) لأن الجدولة الصارمة وحدها تُظهر ~98% في +120.
...
| تعريف أعمار الأقساط | ✅ **محلول** (`discovery/01`): الاستحقاقات مشتقّة والأعمار قابلة للبناء؛ يبقى اختيار المقياس مع المالك |
