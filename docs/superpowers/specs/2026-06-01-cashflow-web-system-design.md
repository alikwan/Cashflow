...
**`installments_aging`** — **⛔ مؤجَّل في v1** (نتيجة `discovery/01`: لا حقول تاريخ استحقاق في المصدر → تُعرض «متبقٍ» واحد بدل الشرائح). يبقى التصميم للمستقبل: PK مركّب **`(snapshot_date, bucket_key)`**، `label text`, `amount_m`, `count int`؛ ومتى توفّرت استحقاقات يفرض ETL Σ(amount_m) = `installments_summary.remaining_m`.
...
- **أعمار الأقساط**: ✅ حُسم في `discovery/01` — **لا حقول استحقاق في المصدر**، فالنسخة الأولى **تُسقط الشرائح** وتكتفي بإجمالي الالتزامات/المُحصّل/المتبقّي + كبار المدينين. (اشتقاق أعمار تقريبية من `InitDate`+`PremiumNo` مؤجَّل لما بعد v1.)
...
| حقول تاريخ استحقاق الأقساط | ✅ **تمّ** (`discovery/01`): لا حقول → أُسقطت الشرائح في v1 |
