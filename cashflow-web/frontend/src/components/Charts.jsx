// البيت السعيد — Charts (lightweight interactive SVG).
//
// Ported verbatim from design-reference/project/src/Charts.jsx. All geometry,
// padding, colors, gradients, tooltip styling and hover behavior are a BINDING
// visual contract and preserved byte-for-byte. The ONLY changes are mechanical:
//   1. React UMD globals → ES import (the aliased useStateC/useRefC/etc. local
//      names are kept to minimize diff risk; they bind to the standard hooks).
//   2. `Object.assign(window, {...})` → named `export` on each component.
// Charts does NOT use Icon (the source's `/* global ..., Icon */` was a stray),
// so it is intentionally not imported. `cssVar` stays a named export.
//
// NOTE: `useWidth` relies on `ResizeObserver`, which jsdom does not implement —
// tests stub it in tests/setup.js.
import React, { useState as useStateC, useRef as useRefC, useEffect as useEffectC, useMemo as useMemoC } from "react";

export function cssVar(v) { return v && v.startsWith('--') ? `var(${v})` : v; }
...
export function LineChart({ series, labels, height = 280, formatY, forecastFrom = null, yZero = true, renderTip, tickEvery = 1 }) {
...
export function BarChart({ data, height = 220, formatY, colorFor, tickEvery = 1, unit = '' }) {
  const [ref, W] = useWidth();
  const [hi, setHi] = useStateC(null);
...
export function StackedBarChart({ data, height = 260, formatY, tickEvery = 1, unit = '' }) {
  const [ref, W] = useWidth();
  const [hi, setHi] = useStateC(null);
  const padT = 12, padB = 28, padR = 8, padL = 50;
  const n = data.length;
...
export function Donut({ segments, size = 180, thickness = 26, centerLabel, centerSub, onHover }) {
  const [hi, setHi] = useStateC(null);
