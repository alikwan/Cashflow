...
| `cashflow-frontend` | nginx يقدّم واجهة React **ويوكّل `/api`→backend** | **8100** (مفتوح على الشبكة) |

**النموذج same-origin:** المتصفّح يكلّم `cashflow-frontend` فقط (المنفذ 8100).
...
| **المنفذ 8100 مشغول** عند الرفع | خدمة أخرى تستخدمه | أوقِف ما يشغله، أو غيّر تخطيط المنفذ في `docker/compose.cashflow.yml` (خدمة `frontend`: `"8100:80"` → منفذ آخر) ثم أعِد `up -d`. |
...
3. **قيّد الوصول على مستوى الشبكة:** المنفذ 8100 مفتوح على شبكة المعرض كاملةً
